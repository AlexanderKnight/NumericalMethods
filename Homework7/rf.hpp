...
class RootFinder
{
  public:
    ...
    vector<double> BracketMinimum(const double (*func)(const double &x), 
                           const double a, const double b,
                           const double tol, const int maxIt);
    double Brent(const double (*func)(const double &x),
  ...
  private:
    ...
    double S;
    double tau;
    double P(void){return 100.*rho0()*rho0()+rho0()*T;}
    double rho0(void){return rho/(SqrtDetg*W);}
    double h(void){return 1.+((2.*P())/rho0());}
};
